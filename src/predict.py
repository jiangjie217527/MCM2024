...
t_max = 16  # end at year 10 (1971)
t_h = 1e-1


# 1961 - 1968 true value
# 1968 - 1976 predict
...
def work(eta):
    ...
    for i in range(t.shape[0] - 1):
        ...
        noise = 4000
        n[i + 1] += rd.uniform(-noise * 10, noise * 10)
        f[i + 1] += rd.uniform(-noise, noise)
        m[i + 1] += rd.uniform(-noise, noise)
    ...
    df.to_excel('output_tmp.xlsx', index=False)

    plt.subplot(1, 2, 1)
    plt.plot(t[0:int(t.shape[0] / 2)], n[0:int(t.shape[0] / 2)], 'b', label='true N_prey', linewidth=2.0)
    plt.plot(t[int(t.shape[0] / 2):t.shape[0] - 1], n[int(t.shape[0] / 2):t.shape[0] - 1], 'r--',
             label='predict N_prey', linewidth=2.0)
    plt.fill_between(
        t,
        n,
        y2=0,
    ...
    plt.axvline(t[int(t.shape[0] / 2) - 1], color='k', linestyle='dashed', linewidth=3.0)
    plt.legend()
    plt.xlabel('t_year')
    plt.ylabel('number')
    plt.title('n-t')
    ...
    plt.subplot(1, 2, 2)
    plt.plot(t[0:int(t.shape[0] / 2)], f[0:int(t.shape[0] / 2)], 'b',
             label='true F', linewidth=2.0)
    plt.plot(t[int(t.shape[0] / 2):t.shape[0] - 1], f[int(t.shape[0] / 2):t.shape[0] - 1], 'r--',
             label='predict F', linewidth=2.0)
    plt.plot(t[0:int(t.shape[0] / 2)], m[0:int(t.shape[0] / 2)], 'b',
             label='true M', linewidth=2.0)
    plt.plot(t[int(t.shape[0] / 2):t.shape[0] - 1], m[int(t.shape[0] / 2):t.shape[0] - 1], 'r--',
             label='predict M', linewidth=2.0)
    plt.fill_between(
        t,
        f,
        y2=0,
